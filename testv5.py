...
def log_event(f, writer, is_csv, etype, msg, details=None):
    ...
    if len(parts) >= 2:
        # Extract AV name from event type (EVENT/Windows Defender, FILE/Bitdefender, XML/Bitdefender, etc.)
        if event_source in ["EVENT", "FILE", "XML"] and len(parts) > 1:
            av_name = parts[1]
    ...
    if event_source in ["AV-NET", "NETWORK"] and domain_name:
        ...
        if update_type == "UPDATE":
            ...
            print(output)
        elif event_source in ["FILE", "EVENT", "XML"]:
            if av_name:
    ...
    if is_csv:
        ...
        if details:
            ...
            elif event_source == "XML" and len(details) >= 6:
                # XML update details: [xml_path, component, updavailable, update_time, size, server, av_name]
                xml_path = details[0] if len(details) > 0 else "N/A"
                component = details[1] if len(details) > 1 else "N/A"
                updavailable = details[2] if len(details) > 2 else "N/A"
                update_time = details[3] if len(details) > 3 else "N/A"
                size = details[4] if len(details) > 4 else "N/A"
                server = details[5] if len(details) > 5 else "N/A"
                av_name_log = details[6] if len(details) > 6 else (av_name or "Unknown")
                
                f.write(f" | XMLPath: {xml_path} | {component} | UpdAvailable: {updavailable} | UpdateTime: {update_time} | Size: {size} | Server: {server} | AV: {av_name_log}")
            else:
...
class BitdefenderXMLMonitor(threading.Thread):
    ...
    def __init__(self, f, writer, is_csv):
        ...
        self.is_csv = is_csv
        self.running = True
        self.xml_path = Path("C:/Program Files/Bitdefender/Bitdefender Security/update_statistics.xml")
    ...
    def parse_xml(self):
        ...
        try:
            ...
            if DEBUG_MODE:
                print(f"[XML MONITOR] Error parsing XML: {e}")
            return None
    ...
    def run(self):
        ...
        if DEBUG_MODE:
            print(f"[XML MONITOR] Started monitoring Bitdefender update_statistics.xml...")
...
def main():
    ...
    # Start Bitdefender XML monitor if Bitdefender is being monitored
    if "Bitdefender" in ANTIVIRUS:
        xml_monitor = BitdefenderXMLMonitor(f, writer, is_csv)
        xml_monitor.start()
        monitors.append(xml_monitor)
